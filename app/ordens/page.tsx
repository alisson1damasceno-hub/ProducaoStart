...
import { ArrowRight, Coins, Eye, MessageSquare, Package, Pencil, PlusCircle, Save, Send, Trash2, X } from "lucide-react";
...
export default function OrdersPage() {
  ...
  const [commentAuthor, setCommentAuthor] = useState("Carlos P.");
  ...
  // Sempre busca a versão mais atualizada da OP que está sendo visualizada
  const viewingOrder = viewing ? orders.find((o) => o.id === viewing.id) ?? null : null;
  ...
  function submitComment(event: FormEvent) {
    event.preventDefault();
    if (!viewingOrder || !commentText.trim() || !commentAuthor.trim()) return;
    addComment(viewingOrder.id, commentAuthor.trim(), commentText.trim());
    setCommentText("");
  }
            ...
            <div className="detail-panel" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                <div>
                  ...
                  <p>{viewingOrder.quantity} un · {stageLabels[viewingOrder.stage]} · {viewingOrder.progress}% · MP {currency.format(orderMaterialCost(viewingOrder))}</p>
                </div>
                <div className="detail-panel-actions">
                  <button className="icon-btn" type="button" onClick={() => startEdit(viewingOrder)} title="Editar OP"><Pencil size={16} /></button>
                  <button className="icon-btn danger" type="button" onClick={() => confirmDelete(viewingOrder)} title="Excluir OP"><Trash2 size={16} /></button>
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  {viewingOrder.comments.length === 0 ? (
                    ...
                    viewingOrder.comments.map((comment) => (
                      <div key={comment.id} style={{
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                          <strong style={{ fontSize: 12 }}>{comment.author}</strong>
                        ...
                        <p style={{ fontSize: 13, margin: 0, lineHeight: 1.5 }}>{comment.text}</p>
                      </div>
                    ))
                  )}
                </div>
                ...
                <form onSubmit={submitComment} style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  <div className="row" style={{ gap: 8 }}>
                    <div className="form-group" style={{ flex: 1, margin: 0 }}>
                      <label style={{ fontSize: 12 }}>Seu nome</label>
                      <input
                        value={commentAuthor}
                        onChange={(e) => setCommentAuthor(e.target.value)}
                        placeholder="Ex: Carlos P."
                        style={{ fontSize: 13 }}
                      />
                    </div>
                  </div>
                  <div className="form-group" style={{ margin: 0 }}>
                    <label style={{ fontSize: 12 }}>Observação / justificativa</label>
                    <textarea
                      value={commentText}
                      onChange={(e) => setCommentText(e.target.value)}
                      placeholder="Digite sua observação, justificativa ou depoimento sobre esta OP..."
                      rows={3}
                      style={{
                        width: "100%",
                        resize: "vertical",
                  ...
                  >
                    <Send size={15} /> Enviar comentário
                  </button>
                </form>
              </div>
                  ...
                  <tr key={order.id}>
                    ...
                    <td>
                      ...
                      {order.comments?.length > 0 && (
                        <span title={`${order.comments.length} comentário(s)`} style={{
                          marginLeft: 6,
                          fontSize: 11,
                    ...
                    <td>
                      <div className="row-actions">
                        <button className="icon-btn" type="button" onClick={() => setViewing(order)} title="Visualizar OP e comentários"><Eye size={16} /></button>

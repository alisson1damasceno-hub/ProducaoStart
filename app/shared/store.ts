...
function normalizeData(data: MvpData): MvpData {
  return {
    ...
    sheets: (data.sheets || []).map((sheet) => {
      const seedSheet = seedData.sheets.find((item) => item.id === sheet.id || item.code === sheet.code);
      return {
        ...sheet,
        rawMaterials: Array.isArray(sheet.rawMaterials) && sheet.rawMaterials.length
          ? sheet.rawMaterials
  ...
  return useMemo(
    () => ({
      ...
      addProduct(product: Omit<Product, "id">) {
        const nextProduct = { ...product, id: createId("prod") };
        clearApiDataCache();
        setData((current) => ({
          ...current,
          products: [nextProduct, ...current.products]
        }));
        apiPost("/api/products", nextProduct).catch((error) => setLastError(error instanceof Error ? error.message : "Erro ao salvar produto."));
      },
      updateProduct(product: Product) {
        ...
        apiPatch(`/api/products/${product.id}`, product).catch((error) => setLastError(error instanceof Error ? error.message : "Erro ao atualizar produto."));
      },
      deleteProduct(productId: string) {
        clearApiDataCache();
        setData((current) => {
          const deletedSheetIds = current.sheets.filter((sheet) => sheet.productId === productId).map((sheet) => sheet.id);
          return {
        ...
        apiDelete(`/api/products/${productId}`).catch((error) => setLastError(error instanceof Error ? error.message : "Erro ao excluir produto."));
      },
      addSheet(sheet: Omit<TechnicalSheet, "id" | "code">) {
        ...
        setData((current) => {
          ...
          apiPost("/api/sheets", nextSheet).catch((error) => setLastError(error instanceof Error ? error.message : "Erro ao salvar ficha."));
      ...
      updateSheet(sheet: TechnicalSheet) {
        ...
        apiPatch(`/api/sheets/${sheet.id}`, sheet).catch((error) => setLastError(error instanceof Error ? error.message : "Erro ao atualizar ficha."));
      },
      deleteSheet(sheetId: string) {
        ...
        apiDelete(`/api/sheets/${sheetId}`).catch((error) => setLastError(error instanceof Error ? error.message : "Erro ao excluir ficha."));
      },
      addOrder(order: Omit<ProductionOrder, "id" | "code" | "lot" | "createdAt" | "stage" | "progress">) {
        ...
        setData((current) => {
          ...
          apiPost("/api/orders", nextOrder).catch((error) => setLastError(error instanceof Error ? error.message : "Erro ao salvar ordem."));
      ...
      updateOrder(order: ProductionOrder) {
        ...
        apiPatch(`/api/orders/${order.id}`, order).catch((error) => setLastError(error instanceof Error ? error.message : "Erro ao atualizar ordem."));
      },
      deleteOrder(orderId: string) {
        ...
        apiDelete(`/api/orders/${orderId}`).catch((error) => setLastError(error instanceof Error ? error.message : "Erro ao excluir ordem."));
      },
      ...
      moveOrder(orderId: string, stage: ProductionOrder["stage"]) {
        ...
        apiPatch(`/api/orders/${orderId}`, { stage, progress: nextProgress }).catch((error) => setLastError(error instanceof Error ? error.message : "Erro ao mover ordem."));
      },
      resetDemo() {
        clearApiDataCache();
        setData(seedData);
